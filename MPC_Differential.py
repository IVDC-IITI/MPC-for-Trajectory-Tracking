...
# mpc parameters
R = np.diag([100, 10])  # input cost matrix
Q = np.diag([100000,100000, 0.000])  # state cost matrix
Qf = np.diag([100000, 100000, 0.000]) # state final matrix
Rd = np.diag([10, 0.1])
...
def calc_speed_profile(cx, cy, cyaw, target_speed):
    ...
    for i in range(len(cx) - 1):
        ...
        dangle = abs(move_direction -cyaw[i])
        if dangle >= math.pi / 2.0:  # Reverse if angle exceeds 90 degrees
            speed_profile[i] = -target_speed
        else:
            speed_profile[i] = target_speed
    speed_profile[-1] = 0.0  # Stop at the end of the path
...
class MPCTrack(Node):
    def __init__(self):
        ...
        # Subscribers
        self.odom_sub = self.create_subscription(Odometry, '/odom', self.odomfunc, 10)
    ...
    def odomfunc(self, msg: Odometry):
        ...
        self.theta = math.atan2(2 * (self.qx * self.qy + self.qw * self.qz), 1 - 2 * (self.qy * self.qy + self.qz * self.qz))

    def main_control_loop(self):
        msg = Twist()

        # ax, ay = csv_to_lists('/home/dishank/Desktop/odom_data.csv')

        dl = 0.1 # course tick
        ax = [-1.998164, -0.3, -0.3, -1.99]
        ay = [-0.499999, -0.5, -2.141250, -2.14]
        ...
        while MAX_TIME >= time:
            ...
            if abs(cyaw[0] - state.yaw) > math.pi/2.0:
                vi = -vi

            msg.linear.x = vi
            msg.angular.z = omegai
